...
$(function () {
    ...
    $.fn.ajaxPost({
        ...
        succeed: function (res) {
            var original = $("#navPanelBar").clone(true);
            ...
            $('#navMenu').kendoMenu({
                ...
                //open: function (e) {
                //    console.log(e);
                //    console.log(location.hash.split('#')[1].split('/')[location.hash.split('#')[1].split('/').length - 1]);
                //    setTimeout(function () {
    ...
    $.fn.ajaxPost({
        ...
        succeed: function (res) {
            ...
            data[data.length - 1].text = "<abbr>" + sessionStorage.getItem('fullname') + "</abbr><img src='" + sessionStorage.getItem('avatar')+"' alt=''>";
    ...
    setTimeout(function () {
        $(window).resize(function () {
            routerPage = location.hash.split('#')[1].split('/')[location.hash.split('#')[1].split('/').length - 1];
            showPath(location.hash.split('#')[1].split('/')[location.hash.split('#')[1].split('/').length - 1]);
        }).resize();
    }, 8);
...
function RoleCheck(_page) {
    ...
    $.fn.ajaxPost({
        ...
        failed: function (res) {
            noticeMsg('访问异常', 'error', 'center', 500, function () {
                logout();
            });
        }
    });
...
function showPath(hash) {
    ...
    $('#path').html('');
    $.each($('#nav').find('.links-'+ hash).children('.k-link').parents('.k-item'), function (i, doms) {
